...
var input = await InputHelper.GetInputAsLines(2023, 1);
...
{
    var first = Parse(RegexHelper.FirstMatch(RegexHelper.Numbers, line));
    var last = Parse(RegexHelper.LastMatch(RegexHelper.Numbers, line));
    sum += first * 10 + last;
}
